...
public: 
  ...
  int   operator!=(const Pnt3 &);
  inline bool operator > ( const Pnt3& rhs ) const
  {
	  if (v[0] > rhs[0])
	  {
		  return true;
  ...
  friend float  dist2_tri_ransac(const Pnt3 &x, 
	  const Pnt3 &t1, const Pnt3 &t2, const Pnt3 &t3);
 
  friend float  angle_plant_vecter(const Pnt3 &, const Pnt3 &, const Pnt3 &);
...
inline float
angle_plant_vecter(const Pnt3 &a, const Pnt3 &b,const Pnt3 &c)
{
	Pnt3 P;

	P = (a+b+c)/3.0;

	float x = (a.v[1]-b.v[1])*(c.v[2]-a.v[2]) - (a.v[2]-b.v[2])*(c.v[1]-a.v[1]);
	float y = (c.v[0]-a.v[0])*(a.v[2]-b.v[2]) - (a.v[0]-b.v[0])*(c.v[2]-a.v[2]);
	float z = (a.v[0]-b.v[0])*(c.v[1]-a.v[1]) - (a.v[1]-b.v[1])*(c.v[0]-a.v[0]);
	return abs(P.v[0]*x+P.v[1]*y+P.v[2]*z)/sqrtf(P.v[0]*P.v[0]+P.v[1]*P.v[1]+P.v[2]*P.v[2])/sqrtf(x*x+y*y+z*z);
...
// get a normal from triangle ABC, points given in ccw order (counter clock wise)
inline Pnt3 
		  ...
		  float &b1, float &b2, float &b3)
{
	...
	if (n.v[2] > n.v[i]) {
		// ignore z
		float d = 1.0f / (v1.v[0]*v2.v[1]-v1.v[1]*v2.v[0]);
		...
		// ignore x
		float d = 1.0f / (v1.v[1]*v2.v[2]-v1.v[2]*v2.v[1]);
		...
		// ignore y
		float d = 1.0f / (v1.v[2]*v2.v[0]-v1.v[0]*v2.v[2]);
...
Pnt3::smallest_circle(const Pnt3 &A, const Pnt3 &B, const Pnt3 &C)
{
	...
	if (da > db && da > dc) {
		...
		if (da >= db + dc) {
			...
			// longest edge
			v[0] = 0.5f * (A.v[0]+B.v[0]);
			v[1] = 0.5f * (A.v[1]+B.v[1]);
			v[2] = 0.5f * (A.v[2]+B.v[2]);
			return 0.5f*sqrtf(da);
		}
	} else if (dc > db) {
		// dc longest
		if (dc >= db + da) {
			// over 90 deg angle, solution is the center of the 
			// longest edge
			v[0] = 0.5f * (A.v[0]+C.v[0]);
			v[1] = 0.5f * (A.v[1]+C.v[1]);
			v[2] = 0.5f * (A.v[2]+C.v[2]);
			return 0.5f*sqrtf(dc);
		}
	} else {
		// db longest
		if (db >= da + dc) {
			// over 90 deg angle, solution is the center of the 
			// longest edge
			v[0] = 0.5f * (B.v[0]+C.v[0]);
			v[1] = 0.5f * (B.v[1]+C.v[1]);
			v[2] = 0.5f * (B.v[2]+C.v[2]);
			return 0.5f*sqrtf(db);
		}
	}
	...
	v[0] = A.v[0] + 0.5f * (a.v[0] + fact * aperp.v[0]);
	v[1] = A.v[1] + 0.5f * (a.v[1] + fact * aperp.v[1]);
	v[2] = A.v[2] + 0.5f * (a.v[2] + fact * aperp.v[2]);
	return 0.5f * sqrtf(da*fact*fact+da);
}
...
{
	float x = float(m[0])*v[0] + float(m[4])*v[1] + float(m[8]) *v[2] + float(m[12]);
	float y = float(m[1])*v[0] + float(m[5])*v[1] + float(m[9]) *v[2] + float(m[13]);
	v[2]    = float(m[2])*v[0] + float(m[6])*v[1] + float(m[10])*v[2] + float(m[14]);
	v[0] = x; v[1] = y;
	return *this;
}
...
Pnt3::invxform(float r[3][3], double t[3])
{
	float tx = v[0] - float(t[0]);
	float ty = v[1] - float(t[1]);
	float tz = v[2] - float(t[2]);
	v[0] = r[0][0]*tx + r[1][0]*ty + r[2][0]*tz;
	v[1] = r[0][1]*tx + r[1][1]*ty + r[2][1]*tz;
	v[2] = r[0][2]*tx + r[1][2]*ty + r[2][2]*tz;
	return *this;
