...
public:
	...
	void apply_nrm_inv_test(const double i[3], double o[3]) const;
	void apply_nrm_inv_test(const float i[3],  float o[3]) const;
	...
	friend Xform<T> operator*<T>(const Xform<T> &a,     // a*b
		const Xform<T> &b); 
	friend Xform<T> delta<T>(Xform<T> a,                // d * a = b
		const Xform<T> &b); 
	friend Xform<T> linear_interp<T>(const Xform<T> &a, // r=t*a+(1-t)*b
		const Xform<T> &b,
		double          t);
	friend Xform<T> bilinear_interp<T>(const Xform<T> &u0v0,
		const Xform<T> &u1v0,
		const Xform<T> &u0v1,
		const Xform<T> &u1v1,
		double u, double v);
	friend Xform<T> relative_xform<T>(const Xform<T> &a, 
		Xform<T> b); // c = b^-1 * a

	friend ostream& operator<<<T>(ostream &out, const Xform<T> &xf);
	friend istream& operator>><T>(istream & in,       Xform<T> &xf);
...
Xform<T>::rot(T angle, T ax, T ay, T az) // M <- R*M
{
	T q[4];
	q[0] = cos(angle/2.0f);
	T fact = sin(angle/2.0f) / sqrt(ax*ax+ay*ay+az*az);
	q[1] = ax*fact;
	q[2] = ay*fact;
	q[3] = az*fact;
...
{
	float invw = 
		1.0f / (m[0][3]*i[0]+m[1][3]*i[1]+m[2][3]*i[2]+m[3][3]);
	o[0] = (m[0][0]*i[0]+m[1][0]*i[1]+m[2][0]*i[2])*invw;
	o[1] = (m[0][1]*i[0]+m[1][1]*i[1]+m[2][1]*i[2])*invw;
	o[2] = (m[0][2]*i[0]+m[1][2]*i[1]+m[2][2]*i[2])*invw;
}
...
template <class T> inline void 
	Xform<T>::apply_nrm_inv_test(const double i[3], double o[3]) const // o = M^-1*i
{
	T tx = i[0];
	T ty = i[1];
	T tz = i[2];
...
template <class T> inline void 
	Xform<T>::apply_nrm_inv_test(const float i[3], float o[3]) const // o = M*i
{
	T tx = i[0];
	T ty = i[1];
	T tz = i[2];
